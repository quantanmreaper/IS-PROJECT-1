                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    {quickActions.map((action, index) => (
                        <Link 
                            key={index} 
                            href={action.href}
                            className={`${action.color} p-6 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 hover:translate-y-[-4px] border border-blue-200 group`}
                        >
        ...
        <AuthenticatedLayout>
            ...
            <div className="py-8 md:py-12">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    {isAdmin ? (
                        <AdminDashboard metrics={metrics?.admin} />
                    ) : (
