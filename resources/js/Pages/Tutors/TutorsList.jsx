...
export default function TutorsList({ tutors }) {
    ...
    // Define common expertise areas
    const expertiseAreas = ["Mathematics", "Physics", "Programming", "Chemistry", "Biology", "History", "Languages"];
    ...
    useEffect(() => {
        ...
        if (selectedExpertise !== "all") {
            // For demonstration purposes, we'll filter based on the mock expertise data
            // In a real implementation, this would filter based on actual tutor expertise data from API
            result = result.filter(tutor => {
                // Since we're using mock data where all tutors have the same skills,
                // we'll use a deterministic approach based on tutor ID to simulate different expertise
                const tutorId = parseInt(tutor.id, 10);
                const mockExpertiseAreas = expertiseAreas.filter((_, index) => (tutorId + index) % expertiseAreas.length < 3);
                return mockExpertiseAreas.includes(selectedExpertise);
            });
        }
    ...
    }, [tutorsList, searchTerm, sortBy, sortOrder, selectedExpertise, expertiseAreas]);
                ...
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="bg-gradient-to-r from-blue-500 to-blue-700 rounded-2xl shadow-xl overflow-hidden mb-8 relative">
                        <div className="px-4 sm:px-8 py-8 sm:py-12 max-w-3xl">
                            ...
                            <p className="mt-3 sm:mt-4 text-base sm:text-lg md:text-xl text-blue-100">
                                Connect with experienced tutors to help you excel in your academic journey
                            </p>
                            <div className="mt-6 sm:mt-8 bg-white rounded-lg p-2 flex flex-col sm:flex-row items-center shadow-lg">
                                ...
                                <button 
                                    ...
                                    onClick={() => {/* Search handling already done via useEffect */}}
                                >
                    ...
                    <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                            {/* Expertise Filter */}
                            <div>
                                <label htmlFor="expertise-filter" className="block text-sm font-medium text-gray-700 mb-2">Filter by Expertise:</label>
                                <select
                                    id="expertise-filter"
                    ...
                    {isLoading ? (
                                ...
                                >
                                    ...
                                    <div className="p-6 flex-1 flex flex-col">
                                        <div className="mb-5 bg-blue-50 p-4 rounded-lg">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {[1, 2, 3].map((_, index) => (
                                                    <div key={index} className={`h-6 w-${16 + index * 4} bg-blue-${100 + index * 50} rounded-full`}></div>
                                                ))}
                                            </div>
                                        </div>
                                        <div className="mb-4">
                            ...
                            {filteredTutors.map((tutor) => (
                                ...
                                >
                                    ...
                                    <div className="relative p-3 bg-gradient-to-r from-blue-500 to-blue-600 flex items-center">
                                        ...
                                        <div>
                                            <h2 className="font-bold text-base text-white">{tutor.name}</h2>
                                    ...
                                    <div className="p-6 flex-1 flex flex-col bg-white">
                                        ...
                                        <div className="mb-5 bg-blue-50 p-4 rounded-lg border border-blue-100">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {(() => {
                                                    // Generate mock expertise areas based on tutor ID for demonstration
                                                    const tutorId = parseInt(tutor.id, 10);
                                                    const tutorExpertise = expertiseAreas.filter((_, index) => 
                                                        (tutorId + index) % expertiseAreas.length < 3
                                                    );
                                                    
                                                    // Only use different shades of blue for consistency
                                                    const colors = [
                                                    ...
                                                    return tutorExpertise.map((skill, index) => (
                                                        <span 
                                                            key={index} 
                                                            className={`px-3 py-1 text-xs font-medium rounded-full border ${colors[index % colors.length]} ${
                                                                selectedExpertise === skill ? 'ring-2 ring-blue-500 ring-offset-1' : ''
                                                            }`}
                                                        >
                                                            {skill}
                                                        </span>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex items-center  rounded-lg p-3 border border-green-100">
                                                ...
                                                <div>
                                                    <span className="text-xs text-green-600">Hourly Rate</span>
                                                    <div className="font-bold text-green-600">
                                                        {tutor.hourly_rate ? `KSh ${tutor.hourly_rate}/hr` : 'Rate not specified'}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-auto pt-4">
                                            ...
                                            >
                                                ...
                                                <svg className="h-4 w-4 ml-2 transform transition-transform duration-200 group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
                            ...
                            <h3 className="mt-5 text-xl font-semibold text-gray-800">No tutors found</h3>
                            <p className="mt-2 text-gray-500 max-w-md mx-auto">
                                We couldn't find any tutors matching your search criteria. Try adjusting your search terms.
                            </p>
                            <button 
                                onClick={() => {
                                    setSearchTerm("");
